...
## 效能評估結果 (N=100000, repeat=5)

| 策略 | 平均耗時 (ms) | 複雜度 |
|:---|---:|:---:|
| Linear Search | 2.748 | O(n) |
| Binary Search | 0.003 | O(log n) |
| Sort + Binary | 10.392 | O(n log n) |

**誰快？** — Binary Search 比 Linear 快 ~900 倍（資料已排序時）。

**排序划不划算？** — 單次搜尋不划算（Sort+Binary 比 Linear 慢 ~4 倍）；多次搜尋才划算。
